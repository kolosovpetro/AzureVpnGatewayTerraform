...
$RootCert = New-SelfSignedCertificate -Type Custom `
    -KeySpec Signature -Subject "CN=$( $AppName )RootCert" -KeyExportPolicy Exportable `
    -HashAlgorithm sha256 -KeyLength 2048 -CertStoreLocation "Cert:\CurrentUser\My" `
    -KeyUsageProperty Sign -KeyUsage CertSign

$ChildCert = New-SelfSignedCertificate -Type Custom -DnsName "$( $AppName )ChildCert" `
    -KeySpec Signature -Subject "CN=$( $AppName )ChildCert" -KeyExportPolicy Exportable -HashAlgorithm sha256 `
